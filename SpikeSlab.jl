...
# ╔═╡ fbeddb10-7aa3-11ec-0d4c-f9dc3d103454
begin
	using JuMP
	using LinearAlgebra
	using Statistics
...
# ╔═╡ 4b3bab28-ce48-4878-9903-788ff415125c
cd("D:/PhD/Second Year/Comp Econ/Project/Giannone_2021")

# ╔═╡ 300af463-8dfe-410d-a819-15b97b1471c1
md"""
...
# ╔═╡ 555a08e6-cecb-4f31-b973-7f70c285fde3
...
# ╔═╡ 4d5b592e-0a95-4156-aab2-96645302eaf4
md"""
Testing the model with simulated data.
"""

# ╔═╡ 615e6ea6-a389-42b6-a955-593693f2fdac
begin
	Nobs= 1000
	Y = randn(Nobs)
...
# ╔═╡ 0d0bae0d-27de-4172-a23e-69239acaf6df
md"""
...
# ╔═╡ 957d5fcb-629e-4804-81be-8822f59f1386
Plots.histogram(store_q)

# ╔═╡ 526181c7-4693-4ffa-bbf4-0bd7729d9e2b
md"""
The next figure, resembling Figure 4.4, shows the fraction of times each variable was incuded in the model.
"""

# ╔═╡ e5e357d1-e9f5-48c1-aecd-a378b4196298
begin
		meanZ=mean(store_z[:,N+1:M]', dims=1)
		GMT.bar(vec(meanZ), figsize=(6,4), colorbar=true, show=true)
end

# ╔═╡ 5c64ca8b-cf61-4bb2-ab64-d6acd6b1efda
md"""
Real world data used in Giannone et al (2021)
"""

# ╔═╡ 5a78d85f-948f-4782-b819-234af2ff9460
begin
	# Import Macro 1 data
	data_mac1 = readdlm(raw"mac1.csv",',' )
	y_mac1= data_mac1[:,end];
	u_mac1=vec(ones(size(data_mac1,1),1))
	x_mac1=data_mac1[:,1:end-1];
end

# ╔═╡ 577541dd-5af3-4a37-8d9a-a723d902b3b7
begin
	store_BM1,store_zM1,store_phiM1,store_qM1,store_R2M1,store_gamM1,store_s2M1,yM1,xM1,uM1,TM1,kM1 =SpikeSlabGLP(y_mac1,x_mac1,u_mac1)
	q_mac1 = store_qM1; 
end

# ╔═╡ d140e808-c3ec-42cf-a880-256bcfb03f67
Plots.histogram(q_mac1)

# ╔═╡ ffab15b7-4f34-42ac-a956-5fdec61179bf
begin
	# Import Macro 2 data
	data = readdlm(raw"mac2.csv", ',' )
	u_mac2 = data[:,1];
	x_mac2 = data[:,2:end-1];
	y_mac2 =data[:,end]; 
end

# ╔═╡ 73778edb-d62f-450a-b9e8-aa390a389cfd
begin
	store_BM2,store_zM2,store_phiM2,store_qM2,store_R2M2,store_gamM2,store_s2M2,yM2,xM2,uM2,TM2,kM2 =SpikeSlabGLP(y_mac2,x_mac2,u_mac2)
	q_mac2 = store_qM2; 
end

# ╔═╡ b7e36cd1-60a4-4771-8631-6b6b16716495
Plots.histogram(q_mac2)

# ╔═╡ e17829f4-9628-4019-a39e-01cb1a4631a4
md"""
Unfortunately, the replicated Spike and Slab results, diverges from the original ones when it comes to the real data used in the paper. Our efforts to understand why the code produces the correct results in the simulated data but not so for the real data were futile.
"""
...
git-tree-sha1 = "4541e10ce2cdddd174c10bb0586f3171a554e474"
